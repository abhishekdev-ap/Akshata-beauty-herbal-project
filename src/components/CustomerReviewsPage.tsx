...
const CustomerReviewsPage: React.FC<CustomerReviewsPageProps> = ({
    ...
    return (
      <div className="flex space-x-1">
        {[1, 2, 3, 4, 5].map((star) => (
          <Star
            key={star}
            className={`${sizeClasses[size]} transition-all duration-300 ${star <= rating
  ...
  return (
    <div className={`max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 transition-all duration-1000 ${isDarkMode ? 'bg-gray-900' : 'bg-gray-50'
      }`}>
      {/* Animated Header */}
        ...
        <div className="relative">
          ...
          <div className="relative">
            ...
            <p className={`transition-colors duration-500 ${isDarkMode ? 'text-gray-300' : 'text-gray-600'
              ...
              See what our valued customers say about AKSHATA BEAUTY HERBAL PARLOUR
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className={`lg:col-span-1 transform transition-all duration-1000 ease-out ${animateStats ? 'translate-x-0 opacity-100' : '-translate-x-8 opacity-0'
          }`}>
          <div className={`rounded-2xl shadow-lg p-6 sticky top-4 transition-all duration-500 hover:shadow-2xl ${isDarkMode ? 'bg-gray-800 border border-gray-700' : 'bg-white'
            }`}>
            {/* Overall Rating with Animation */}
            <div className="text-center mb-6">
              <div className={`text-4xl font-bold mb-2 animate-bounce transition-colors duration-500 ${isDarkMode ? 'text-pink-400' : 'text-pink-600'
                }`}>
            ...
            <div className="mb-6">
              ...
              {ratingDistribution.map(({ rating, count, percentage }, index) => (
                <div
                  key={rating}
                  className={`flex items-center space-x-2 mb-2 transform transition-all duration-700 hover:scale-105 ${animateStats ? 'translate-x-0 opacity-100' : 'translate-x-4 opacity-0'
                    }`}
                  style={{ transitionDelay: `${index * 100}ms` }}
                >
                  <span className={`text-sm font-medium w-3 transition-colors duration-500 ${isDarkMode ? 'text-gray-300' : 'text-gray-700'
                    }`}>
                    {rating}
                  </span>
                  <Star className="w-4 h-4 text-yellow-400 fill-current animate-pulse" />
                  <div className={`flex-1 h-2 rounded-full transition-colors duration-500 ${isDarkMode ? 'bg-gray-700' : 'bg-gray-200'
                    }`}>
                    <div
                  ...
                  <span className={`text-sm w-8 transition-colors duration-500 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'
                    }`}>
                    {count}
                  </span>
                </div>
            ...
            <div className="space-y-4">
              {[
                ...
                <div
                  key={index}
                  className={`flex items-center space-x-3 p-3 rounded-lg transition-all duration-700 hover:scale-105 cursor-pointer ${stat.color === 'pink' ? (isDarkMode ? 'bg-pink-900/30 border border-pink-700 hover:bg-pink-900/50' : 'bg-pink-50 hover:bg-pink-100') :
                ...
                >
                  <stat.icon className={`w-5 h-5 animate-pulse ${stat.color === 'pink' ? 'text-pink-600' :
                    stat.color === 'purple' ? 'text-purple-600' :
                      'text-green-600'
                    }`} />
                  <div>
                    <div className={`font-semibold transition-colors duration-500 ${stat.color === 'pink' ? (isDarkMode ? 'text-pink-300' : 'text-pink-800') :
                      stat.color === 'purple' ? (isDarkMode ? 'text-purple-300' : 'text-purple-800') :
                        (isDarkMode ? 'text-green-300' : 'text-green-800')
                      }`}>
                      {stat.title}
                    </div>
                    <div className={`text-sm transition-colors duration-500 ${stat.color === 'pink' ? (isDarkMode ? 'text-pink-400' : 'text-pink-600') :
            ...
            <div className={`mt-6 p-4 rounded-xl border transition-all duration-1000 hover:scale-105 ${isDarkMode
              ...
              <p className={`text-sm mb-3 transition-colors duration-500 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'
                }`}>
                Help others by sharing your experience at AKSHATA BEAUTY HERBAL PARLOUR
              </p>
              <button
                onClick={onWriteReview}
                className={`w-full py-2 rounded-lg font-medium transition-all duration-300 flex items-center justify-center space-x-2 hover:scale-105 ${isDarkMode
        ...
        <div className={`lg:col-span-3 transform transition-all duration-1000 ease-out ${animateReviews ? 'translate-x-0 opacity-100' : 'translate-x-8 opacity-0'
          }`}>
          {mockReviews.length > 0 ? (
            <>
              {/* Animated Filters */}
              <div className={`rounded-xl shadow-md p-4 mb-6 transition-all duration-500 hover:shadow-lg ${isDarkMode ? 'bg-gray-800 border border-gray-700' : 'bg-white'
                }`}>
                <div className="flex flex-wrap items-center justify-between gap-4">
                  <div className="flex items-center space-x-4">
                    <Filter className={`w-5 h-5 animate-pulse transition-colors duration-500 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'
                      }`} />
                    ...
                    <div className="flex space-x-2">
                      <button
                        ...
                        className={`px-3 py-1 rounded-full text-sm font-medium transition-all duration-300 hover:scale-105 ${filterRating === null
                      ...
                      {[5, 4, 3, 2, 1].map(rating => (
                        <button
                          ...
                          className={`px-3 py-1 rounded-full text-sm font-medium transition-all duration-300 hover:scale-105 ${filterRating === rating
                            ? 'bg-pink-600 text-white shadow-lg'
                            : isDarkMode
                              ? 'bg-gray-700 text-gray-300 hover:bg-pink-900/50'
                              : 'bg-gray-100 text-gray-600 hover:bg-pink-100'
                  ...
                  <select
                    ...
                    className={`px-3 py-2 border rounded-lg focus:ring-2 focus:ring-pink-500 focus:border-transparent transition-all duration-300 ${isDarkMode
              ...
              <div className="space-y-6">
                {filteredReviews.map((review, index) => (
                  <div
                    key={review.id}
                    className={`rounded-xl shadow-md hover:shadow-xl transition-all duration-500 p-6 transform hover:scale-105 cursor-pointer ${isDarkMode ? 'bg-gray-800 border border-gray-700' : 'bg-white'
                  ...
                  >
                    <div className="flex items-start space-x-4">
                      ...
                      <div className="flex-shrink-0">
                        <div className={`w-12 h-12 bg-gradient-to-r ${getAvatarColor(review.customerName || 'User')} rounded-full flex items-center justify-center border-2 border-white shadow-md transition-all duration-300 ${hoveredReview === review.id ? 'scale-110 rotate-12' : ''
                          }`}>
                      ...
                      <div className="flex-1">
                        <div className="flex items-center justify-between mb-2">
                          <div>
                            <div className="flex items-center space-x-2">
                              <h3 className={`font-semibold transition-colors duration-500 ${isDarkMode ? 'text-gray-200' : 'text-gray-800'
                                }`}>
                          ...
                          <div className="flex items-center space-x-2">
                            <div className={`transform transition-all duration-300 ${hoveredReview === review.id ? 'scale-110' : ''
                              }`}>
                            ...
                            {canEditReview(review) && (
                              <button
                                ...
                                className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-all duration-300 hover:scale-110 ml-4"
                                title="Edit review"
                              >
                          ...
                          <div className="mb-3">
                            <div className="flex flex-wrap gap-2">
                              {review.services.map((service, serviceIndex) => (
                                <span
                                  key={serviceIndex}
                                  className={`px-2 py-1 text-xs rounded-full transition-all duration-500 hover:scale-105 ${isDarkMode
                        ...
                        <p className={`leading-relaxed transition-all duration-500 ${isDarkMode ? 'text-gray-300' : 'text-gray-700'
            ...
            <div className={`text-center py-16 transform transition-all duration-1000 ${animateReviews ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'
              }`}>
              ...
              <p className={`mb-8 max-w-md mx-auto transition-colors duration-500 ${isDarkMode ? 'text-gray-500' : 'text-gray-500'
                ...
                Be the first to share your experience at AKSHATA BEAUTY HERBAL PARLOUR! Your review will help other customers and improve our services.
              </p>
              <button
                onClick={onWriteReview}
