...
import { User, LogOut, Sparkles, ChevronDown, Settings, Mail, Calendar, Clock, Receipt, LayoutDashboard } from 'lucide-react';
    ...
    <header className={`${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-pink-100'} shadow-lg border-b sticky top-0 z-50 transition-all duration-300`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          {/* Logo */}
          <div className="flex items-center space-x-2 sm:space-x-3 max-w-[70%] sm:max-w-none">
            <div className="flex-shrink-0 flex items-center justify-center w-8 h-8 sm:w-10 sm:h-10 bg-gradient-to-r from-pink-500 to-purple-600 rounded-full shadow-md">
              <Sparkles className="w-5 h-5 sm:w-6 sm:h-6 text-white" />
            </div>
            <div className="flex flex-col justify-center min-w-0">
              {/* Desktop View - H1 */}
              <h1 className="hidden sm:block text-xl font-bold bg-gradient-to-r from-pink-600 to-purple-600 bg-clip-text text-transparent truncate leading-tight tracking-wide">
                AKSHATA BEAUTY HERBAL PARLOUR
              </h1>
              {/* Mobile View - H4 */}
              <h4 className="block sm:hidden text-xs font-bold bg-gradient-to-r from-pink-600 to-purple-600 bg-clip-text text-transparent truncate leading-tight tracking-wide">
                AKSHATA BEAUTY HERBAL PARLOUR
              </h4>
              <p className={`text-[9px] sm:text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'} block truncate leading-none mt-0.5`}>Beauty & Bridal Services</p>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-3">
            ...
            <div className="relative">
              <button
                ...
                className={`flex items-center space-x-3 p-2 rounded-xl transition-all duration-200 border ${isDarkMode
                ...
                <div className={`absolute right-0 mt-2 w-80 ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} rounded-2xl shadow-xl border py-2 z-50 animate-in slide-in-from-top-2 duration-200`}>
                  ...
                  <div className="py-2">
                    <button
                      ...
                      className={`w-full px-6 py-3 text-left text-sm transition-all duration-300 flex items-center space-x-3 group ${isDarkMode
                        ? 'text-gray-300 hover:bg-gray-700 hover:text-pink-400'
                        : 'text-gray-700 hover:bg-pink-50 hover:text-pink-600'
                        }`}
                    >
                      <Settings className={`w-4 h-4 transition-all duration-300 group-hover:scale-110 ${isDarkMode ? 'group-hover:text-pink-400' : 'group-hover:text-pink-600'
                        }`} />
                      <div>
                        <div className={`font-medium transition-colors duration-300 ${isDarkMode ? 'group-hover:text-pink-400' : 'group-hover:text-pink-600'
                    ...
                    >
                      <Receipt className={`w-4 h-4 transition-all duration-300 group-hover:scale-110 ${isDarkMode ? 'group-hover:text-blue-400' : 'group-hover:text-blue-600'
                        }`} />
                      <div>
                        <div className={`font-medium transition-colors duration-300 ${isDarkMode ? 'text-gray-200 group-hover:text-blue-400' : 'text-gray-800 group-hover:text-blue-600'
                          }`}>Payment History</div>
                        <div className={`text-xs transition-colors duration-300 ${isDarkMode ? 'text-gray-400 group-hover:text-blue-300' : 'text-gray-500 group-hover:text-blue-500'
                          }`}>
                    ...
                    {isUserAdmin && onAdminDashboard && (
                      ...
                      >
                        <LayoutDashboard className={`w-4 h-4 transition-all duration-300 group-hover:scale-110 ${isDarkMode ? 'group-hover:text-purple-400' : 'group-hover:text-purple-600'
                          }`} />
                        <div>
                          <div className={`font-medium transition-colors duration-300 ${isDarkMode ? 'text-gray-200 group-hover:text-purple-400' : 'text-gray-800 group-hover:text-purple-600'
                            }`}>Admin Dashboard</div>
                          <div className={`text-xs transition-colors duration-300 ${isDarkMode ? 'text-gray-400 group-hover:text-purple-300' : 'text-gray-500 group-hover:text-purple-500'
                            }`}>
                    ...
                    <button
                      ...
                      disabled={isSigningOut}
                      className={`w-full px-6 py-3 text-left text-sm text-red-600 hover:bg-red-50 transition-all duration-300 flex items-center space-x-3 disabled:opacity-50 disabled:cursor-not-allowed group ${isDarkMode ? 'hover:bg-red-900/20' : ''
                        }`}
                    >
                      {isSigningOut ? (
                        <>
                  ...
                  <div className={`px-6 py-3 border-t ${isDarkMode ? 'border-gray-700 bg-gradient-to-r from-pink-900/20 to-purple-900/20' : 'border-gray-100 bg-gradient-to-r from-pink-50 to-purple-50'} rounded-b-2xl`}>
                    <p className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-600'} text-center`}>
                      Welcome to <span className="font-semibold text-pink-600">AKSHATA BEAUTY HERBAL PARLOUR</span>
                    </p>
